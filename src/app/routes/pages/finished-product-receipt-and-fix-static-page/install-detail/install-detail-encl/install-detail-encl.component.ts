...
@Component({
  selector: 'app-install-detail-encl',
  templateUrl: './install-detail-encl.component.html',
})
export class InstallDetailEnclComponent implements OnInit {
  ...
  async searchData(reset: boolean = false) {
    ...
    this.data = await this.warehousingFixStaticService.fetchListByMonthForEncl(this.listQuery);
  ...
  exportReport(date:string) {
    window.open(`/api/sites/records/containment/download?${this.utilService.objToSearch({queryDate:date, monomerId: this.currentMonomer})}`);
  }
