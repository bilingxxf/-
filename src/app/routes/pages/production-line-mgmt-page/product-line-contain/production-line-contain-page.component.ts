...
@Component({
  selector: 'app-production-line-contain-page',
  templateUrl: './production-line-contain-page.component.html',
})
export class ProductionLineContainComponent implements OnInit {
    ...
    async navigateTo(id:number){
        console.log(id)
        this.dto.id = id 
        this.router.navigate(['product-contain-detail'], {
            queryParams: {
                lineId: id
              }
        })
    ...
    async c(row?: Role) {
        ...
        try {
            if (row && row.id) {
                ...
                console.log('ssss')
                await this.roleService.addLine(name, 1)
