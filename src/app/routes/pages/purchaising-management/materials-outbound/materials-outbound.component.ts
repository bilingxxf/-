...
@Component({
  selector: 'app-materials-outbound',
  providers: [MaterialDataService],
  templateUrl: './materials-outbound.component.html',
})
export class MaterialsOutboundComponent implements OnInit {
    ...
    ngOnInit() {
      this.userService.user
        ...
        .subscribe((user: any) => {
          ...
          this.getMaterialOutboundOrder();
        })
    }

    getMaterialOutboundOrder(next = 1) {
      this.materialDataQuery.page = next;
      this.materialDataService.getMaterialOutboundOrder(this.materialDataQuery).subscribe()
    }
